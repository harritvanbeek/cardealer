...
Citizen.CreateThread(function()
    Citizen.SetTimeout(1000, function()
        exports.oxmysql:execute("SELECT * FROM `server-cars` WHERE `type` = 'premium'", function(result)
...
AddEventHandler('pepe-cardealer:server:buy:current:vehicle', function(CurrentSlot)
    ...
    if Config.PremiumDealerSpots[CurrentSlot]['Stock'] > 0 then
        if Player.Functions.RemoveMoney("bank", Config.PremiumDealerSpots[CurrentSlot]['Price'], "vehicle-shop") then
            ...
            Framework.Functions.UpdateSql(false, "UPDATE `server-cars` SET stock = '"..NewStock.."' WHERE `vehiclename` = '"..Config.PremiumDealerSpots[CurrentSlot]['Model'].."'")
            Framework.Functions.InsertSql(false, "INSERT INTO `characters_vehicles` (`citizenid`, `vehicle`, `plate`, `garage`, `state`, `mods`, `metadata`) VALUES ('"..Player.PlayerData.citizenid.."', '"..Config.PremiumDealerSpots[CurrentSlot]['Model'].."', '"..Plate.."', '"..GarageData.."', 'out', '{}', '"..json.encode(VehicleMeta).."')")
            ...
            TriggerClientEvent('Framework:Notify', src, "You dont have enough money", 'error', 5500)
        end
    else
        TriggerClientEvent('Framework:Notify', src, "Vehicle not in stock", 'error', 5500)
    end
end)
